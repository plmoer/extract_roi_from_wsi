...
from torch.nn import Module, Sequential, Conv3d, ReLU, AdaptiveAvgPool3d, BCELoss, CrossEntropyLoss
...
class SeparableConv3d(nn.Module):
    def __init__(self, inplanes, planes, kernel_size=3, stride=1, padding=1, dilation=1, bias=False, BatchNorm=nn.GroupNorm):
        super(SeparableConv3d, self).__init__()

        self.conv1 = nn.Conv3d(inplanes, inplanes, kernel_size,
                               stride, padding, dilation, groups=inplanes)
        self.bn = BatchNorm(4, inplanes)
        self.pointwise = nn.Conv3d(inplanes, planes, 1, 1, 0, 1, 1)
...
class JPU(nn.Module):
    def __init__(self, in_channels, width=512, norm_layer=None, up_kwargs=None):
        ...
        self.conv5 = nn.Sequential(
            nn.Conv3d(in_channels[-1], width, 3, padding=1),
            nn.GroupNorm(4, width),
            # norm_layer(width),
            nn.LeakyReLU(0.01))
        # nn.ReLU(inplace=True))
        self.conv4 = nn.Sequential(
            nn.Conv3d(in_channels[-2], width, 3, padding=1),
            # norm_layer(width),
            nn.GroupNorm(4, width),
            nn.LeakyReLU(0.01))
        # nn.ReLU(inplace=True))
        self.conv3 = nn.Sequential(
            nn.Conv3d(in_channels[-3], width, 3, padding=1),
            # norm_layer(width),
            nn.GroupNorm(4, width),
            nn.LeakyReLU(0.01))
        # nn.ReLU(inplace=True))

        self.dilation1 = nn.Sequential(SeparableConv3d(3*width, width, kernel_size=3, padding=1, dilation=1, bias=False),
                                       #    norm_layer(width),
                                       nn.GroupNorm(4, width),
                                       nn.ReLU(inplace=True))
        self.dilation2 = nn.Sequential(SeparableConv3d(3*width, width, kernel_size=3, padding=2, dilation=2, bias=False),
                                       #    norm_layer(width),
                                       nn.GroupNorm(4, width),
                                       nn.ReLU(inplace=True))
        self.dilation3 = nn.Sequential(SeparableConv3d(3*width, width, kernel_size=3, padding=4, dilation=4, bias=False),
                                       #    norm_layer(width),
                                       nn.GroupNorm(4, width),
                                       nn.ReLU(inplace=True))
        self.dilation4 = nn.Sequential(SeparableConv3d(3*width, width, kernel_size=3, padding=8, dilation=8, bias=False),
                                       #    norm_layer(width),
                                       nn.ReLU(inplace=True))
    ...
    def forward(self, *inputs):
        feats = [self.conv5(inputs[-1]), self.conv4(inputs[-2]),
                 self.conv3(inputs[-3])]
        _, _, h, w, d = feats[-1].size()
        feats[-2] = nn.functional.interpolate(feats[-2],
                                              (h, w, d), mode='trilinear', align_corners=False)
        feats[-3] = nn.functional.interpolate(feats[-3],
                                              (h, w, d), mode='trilinear', align_corners=False)
        # feats[-2] = F.upsample(feats[-2], (h, w, d), **self.up_kwargs)
        # feats[-3] = F.upsample(feats[-3], (h, w, d), **self.up_kwargs)
