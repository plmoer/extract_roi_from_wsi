...
__all__ = ['FCN', 'get_fcn',
           'get_fcn_resnet50_pcontext', 'get_fcn_resnet50_ade']
...
class FCN(BaseNet):
    ...
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm3d, **kwargs):
        super(FCN, self).__init__(nclass, backbone, aux,
                                  se_loss, norm_layer=norm_layer, **kwargs)
...
class FCNHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer):
        ...
        self.conv5 = nn.Sequential(nn.Conv3d(in_channels, inter_channels, 3, padding=1),
                                   nn.GroupNorm(4, inter_channels),
                                   #    norm_layer(inter_channels),
                                   nn.ReLU(),
                                   nn.Dropout3d(0.1, False),
                                   nn.Conv3d(inter_channels, out_channels, 1))
...
def get_fcn(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = FCN(datasets[dataset.lower()].NUM_CLASS,
                backbone=backbone, root=root, **kwargs)
    if pretrained:
