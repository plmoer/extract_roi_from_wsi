...
model_path = os.path.join(rootDir, 'models_mri_valid/')
...
trainPatchInfoPath = os.path.join(rootDir, 'trainList.txt')
train_dataset = train_DriveData(
...
validation_dataset = valid_DriveData(
    train_dataFolderDir, validationPatchInfoPath,  train_info,  transformations_validation)
...
def test():
    ...
    for batch_idx,  (data, target) in enumerate(validation_loader):
        ...
        loss = loss + torch.sum(torch.abs(seLoss)) + factor*reg_loss
        valid_loss = valid_loss+loss.data.cpu().numpy()
...
for epoch in range(0, nTotalEpoch):
    ...
    print('=====>The train loss: %.4f train dice: %.4f, ....., old validation dice: %.4f and new validation dice: %.4f' %
          (train_ave_loss, train_ave_dice, nDice, valid_ave_dice))
    if valid_ave_dice > nDice:
        # nDice = valid_ave_dice  # update the validation accuracy
        print('*********The updated average valid old dice: %.4f, new dice: %.4f' %
              (nDice, valid_ave_dice))
        nDice = valid_ave_dice  # update the validation accuracy
